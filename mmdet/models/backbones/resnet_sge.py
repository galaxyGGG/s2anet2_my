...
class SpatialGroupEnhance(nn.Module):
    def __init__(self, groups):
        ...
        self.groups = groups
        self.avg_pool = nn.AdaptiveAvgPool2d(1)  # AdaptiveAvgPool2d(1)即GAP
    ...
    def forward(self, x):  # (b, c, h, w)
        ...
        xn = x * self.avg_pool(x)   # 我个人理解这里类似通道注意力 (b*32, c', h, w)
        xn = xn.sum(dim=1, keepdim=True)  # (b*32, 1, h, w)     # 这里类似空间注意力，空间注意力是在分组中进行的，通过相加完成通道的浓缩
